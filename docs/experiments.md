...
**Pre-acquired connections and a barrier.** Every worker in a trial gets its own dedicated connection from a pool *before* the race starts (`labPool`, sized `LAB_POOL_SIZE`). All workers then wait on one shared promise and are released at the same instant. Without this, requests would queue at the connection pool one at a time and the race window would disappear: nothing would actually run concurrently.
