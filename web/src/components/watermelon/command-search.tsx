// Vendored from https://registry.watermelon.sh/r/command-search.json (2026-09-24). Edits: see git diff.
'use client';
...
export const CommandSearch: FC<Props> = ({ items = DEFAULT_ITEMS, placeholder = 'Find...', open, onOpenChange, hideTrigger }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: any) => {
      if (e.key.toLowerCase() === 'k' && (e.metaKey || e.ctrlKey) && !isOpen) {
        e.preventDefault();
        setIsOpen(true);
      }
          ...
          {!isOpen ? (hideTrigger ? null : (
            ...
            >
              ...
              <motion.kbd
                ...
                transition={sharedTransition}
                className="absolute right-2 rounded border border-zinc-200 bg-zinc-50 px-2 py-0.5 text-[11px] font-bold text-zinc-400 group-hover:text-zinc-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-500 dark:group-hover:text-zinc-300"
              >
                ⌘K
              </motion.kbd>
            </motion.button>
